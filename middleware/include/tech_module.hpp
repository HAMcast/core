...
struct hc_module_handle
{
    ...
    volatile std::int32_t ref_count;
...
class tech_module
{
    ...
    std::uint32_t m_atomic_msg_size;
    ...
    //util::uri_mappings<std::mutex> m_mappings;
 ...
 public:
    ...
    struct tm_release
    {
        inline void operator()(tech_module* tm)
        {
            if (tm)
            {
                std::int32_t rc = hamcast::util::add_and_fetch(&(tm->ref_count), -1);
                if (rc == 0)
                {
    ...
    void group_set(std::vector<std::pair<hamcast::uri, std::uint32_t> >& res);
