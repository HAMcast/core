...
bool client_compatible(const io_ptrs& io, std::uint32_t max_msg_size)
{
    HC_LOG_TRACE("");
    std::uint8_t result = verify_client(io.first) ? 1 : 0;
    // lifetime scope of s
    {
...
void wait4clients_loop(std::uint32_t max_msg_size)
{
    HC_LOG_TRACE("");
...
int run_middleware(const std::string& ini_file)
{
    ...
    // lifetime scope of ini
    {
        ...
        std::uint32_t max_msg_size = default_max_msg_size;
