...
void mh_release::operator()(hc_module_handle* mh)
{
    if (mh)
    {
        std::int32_t rc = hamcast::util::add_and_fetch(&(mh->ref_count), -1);
        if (rc == 0)
        {
            mh->shutdown_fun();
...
typedef std::lock_guard<rw_spinlock> read_guard;
typedef std::lock_guard<rw_spinlock> write_guard;
                                          ...
                                          size_t new_atomic_size)
{
    ...
    else
    {
        ...
        std::lock_guard<std::mutex> guard(tm->m_mtx);
        tm->m_atomic_msg_size = static_cast<std::uint32_t>(new_atomic_size);
    }
}

std::uint32_t tech_module::atomic_msg_size()
{
                          ...
                          size_t init_atomic_size)
{
    ...
    add_instance(tm);
    tm->m_worker = std::thread(&tech_module::run_worker, tm);
    return tm.get();
}
