...
template<typename T>
typename std::enable_if<std::is_integral<T>::value, serializer&>::type
...
{
    return (s << static_cast<std::uint8_t>(what ? 1 : 0));
}

template<typename T>
typename std::enable_if<std::is_integral<T>::value, deserializer&>::type
...
{
    s << (static_cast<std::uint32_t>(what.size()));
...
template<typename Container>
inline void reserve_storage(Container&, std::uint32_t) { }

template<typename T>
inline void reserve_storage(std::vector<T>& vec, std::uint32_t vec_size)
{
