...
class message : public ref_counted, util::comparable<message, message>
{
 ...
 public:
    ...
    inline message(std::uint16_t mtype,
                   std::uint16_t mfield1,
                   std::uint32_t mfield2,
                   std::uint32_t mfield3,
                   std::uint32_t ct_size,
                   char* ct,
    ...
    {
        HC_REQUIRE(ct.first < std::numeric_limits<std::uint32_t>::max());
        return create(mtype, field1, field2, field3,
                      static_cast<std::uint32_t>(ct.first),
     ...
     */
    inline std::uint32_t content_size() const { return m_content_size; }
...
//      HC_REQUIRE(msg_ref && msg_ref->type() == mtype);
 ...
 public:
     ...
     * @returns The size of {@link content()} in bytes.
     */
    inline std::uint32_t content_size() const { return msg.content_size(); }
