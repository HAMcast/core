 ...
 */
void set_ttl(socket_id sid, std::uint8_t value) { }
 ...
 */
std::vector<std::pair<uri, std::uint32_t> > group_set(interface_id iid) { }
 ...
 */
std::uint32_t get_atomic_msg_size(interface_id iid) { }
...
typedef sync_function<fid_set_ttl, util::unit, socket_id, std::uint8_t>
        set_ttl_t;
...
typedef sync_function<fid_group_set, std::vector<std::pair<uri, std::uint32_t> >, interface_id>
        group_set_t;
...
typedef sync_function<fid_get_atomic_msg_size, std::uint32_t, interface_id>
...
class sync_function_dispatcher
{
 ...
 public:
    ...
    void invoke(function_id fid, request_id rid,
                const void* content, std::uint32_t content_size,
                util::serializer& ipc_out)
    {
