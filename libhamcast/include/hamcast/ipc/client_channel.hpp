...
class client_channel : public channel
{
 ...
 protected:
    ...
    typedef std::map<std::uint32_t, sync_response_view> responses_map;
 ...
 public:
    ...
    typedef std::function<void (const uri&, size_t, const void*)> receive_callback;

    inline std::uint32_t max_msg_size() const { return m_max_msg_size; }
     ...
     */
    void send_async_data(socket_id sock, std::uint16_t stream,
                         std::uint32_t ct_size, char* ct);
    ...
    std::uint16_t m_msg_type_field;
    std::uint16_t m_field1;
    std::uint32_t m_field2;
    std::uint32_t m_field3;
    std::uint32_t m_content_size;
