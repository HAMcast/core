...
struct out_stream
{
 ...
 public:
    ...
    void set_id(std::uint16_t new_id)
    {
        m_valid = true;
        m_id = new_id;
    }
...
struct multicast_socket_private : public ref_counted
{
    ...
    void send(const uri& u, size_t msg_len, const void* buf)
    {
        ...
        std::uint32_t buf_len = static_cast<std::uint32_t>(msg_len);
        if (!u.empty())
        {
            ...
            m_out_buffer.acquire(std::max(buf_len, static_cast<std::uint32_t>(min_buffer_chunk)));
            // send
            m_cc->send_async_data(m_id, ostm.id(), buf_len, buf_copy);
        }
    }

    inline std::uint32_t acquire_size(const multicast_packet& mp)
    {
        return mp.size();
    }
    ...
    void release_storage(std::int32_t bytes)
    {
        m_out_buffer.release(std::max(bytes, static_cast<std::int32_t>(min_buffer_chunk)));
    }
    ...
    inline bool try_receive(multicast_packet& mp,
                            std::uint16_t msecs)
    {
        mcast_packet_element* e = m_in_stream.try_pop(std::chrono::milliseconds(msecs));
        if (e)
        {
            m_in_buffer.release(acquire_size(e->mcast_packet));
            mp = e->mcast_packet;
...
{
    std::uint32_t my_id = ipc::create_socket();
...
void multicast_socket::set_ttl(std::uint32_t value)
{
