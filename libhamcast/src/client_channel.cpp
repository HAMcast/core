...
std::uint32_t drop_smaller_or_equal_seq_nrs(socket_data& ostm,
                                              std::uint32_t seq_nr)
{
...
//			result += i->second->content_size;
            result += std::max(i->second->content_size, static_cast<std::uint32_t>(min_buffer_chunk));
            ++removed_items;
        }
        mb.erase(begin, end);
...
{
    std::lock_guard<std::mutex> guard(m_instance_mtx);
    if (!m_instance)
    {
                ...
                switch (mtype)
                {
                 ...
                 case cumulative_ack:
                 {
                    ...
                    if (i != m_sockets.end())
                    {
                        util::serializer s(m_sink);
                        socket_data& sdata = i->second;
                        std::uint32_t released = drop_smaller_or_equal_seq_nrs(sdata, seq_nr);
         ...
         case osm_t:
            {
                ...
                if (!do_not_send)
                {
                    ...
                    message::serialize_as_message(s,
                                                  ...
                                                  rid,
                                                  0,
                                                  static_cast<std::uint32_t>(osm->buf->size()),
                                                  osm->buf->data());
                    s.flush();
...
void client_channel::send_async_data(socket_id sock, std::uint16_t stream,
