...
{
    s << static_cast<std::uint16_t>(m.m_type)
...
void message::serialize_as_message(util::serializer& s,
                                   ...
                                   const void* ct)
{
    s << static_cast<std::uint16_t>(mtype)
...
{
    // header
    s << static_cast<std::uint16_t>(async_event)
      << static_cast<std::uint16_t>(event.type())
      << static_cast<std::uint32_t>(event.iface_id())
      << static_cast<std::uint32_t>(0) // field3 unused
...
{
    s // type
      << static_cast<std::uint16_t>(async_recv)
      // field1 - unused
      ...
      << static_cast<std::uint32_t>(pck.size());
