...
//std::regex m_authority_rx("(?:([a-zA-Z0-9:_\\.]+)@)?([a-zA-Z0-9_\\.]+|\\[[a-fA-F0-9:\\.]+])(?:[:]([0-9]+))?");
std::regex m_authority_rx("*");
std::regex m_v4_rx(HC_OCTET HC_DOT HC_OCTET HC_DOT HC_OCTET HC_DOT HC_OCTET);
...
class uri_private : public ref_counted
{
    ...
    bool parse(const std::string& what)
    {
        ...
        if (parse_uri(what)
            && (m_authority.empty()
                || std::regex_match(m_authority, sm, m_authority_rx)))
        {
            if (!m_authority.empty())
            {
                ...
                else if (!m_host.empty() && std::regex_match(m_host, m_v4_rx))
                {
                    m_flag = ipv4_flag;
                }
                m_port = sm[3];
                ...
                {
                    m_int_port =
                            static_cast<std::uint16_t>(atoi(m_port.c_str()));
                }
            }
...
public:
    ...
    inline std::uint16_t port_as_int() const { return m_int_port; }
...
std::uint16_t uri::port_as_int() const
{
